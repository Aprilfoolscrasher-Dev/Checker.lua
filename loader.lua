...
-- 🔒 STRONG HWID (stable but unique enough)
...
-- 🔁 MULTI EXEC DETECTION
if getgenv().__LOADED then
    log("⚠️ MULTI EXECUTION", 0xffff00, {
...
if not request or not HttpService then
    log("⚠️ ENVIRONMENT TAMPER", 0xff0000, {
        {name="User", value=tostring(player.UserId)}
    })
    return
end
...
-- ⚡ FAST EXEC DETECT (bypass attempt)
